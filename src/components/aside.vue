<template>
  <aside class="aside-container d-flex flex-column justify-content-center align-items-center py-2 px-4 rounded">
    <h3 class="mb-4 category-title">Categorías</h3>
    <div v-for="category in categories" :key="category.id" class="form-check mb-2 me-4" style="width: 100%;">
      <input class="form-check-input" type="checkbox" v-model="selectedCategories" :value="category.nombre" @change="handleCategoryChange(category)">
      <label class="form-check-label">{{ category.nombre }}</label>
    </div>
  </aside>
</template>

<script setup>
import { ref, onBeforeMount, defineEmits } from 'vue';
import { useCategoriasStore } from '../stores/categorias';

const CategoriasStore = useCategoriasStore();
const categories = ref([]);
const selectedCategories = ref([]);
const emit = defineEmits(['nombreCategorias', 'cambiarCategorias']); 

onBeforeMount(async () => {
  categories.value = await CategoriasStore.cargarCategoriasDesdeAPI();
});

const handleCategoryChange = (category) => {
  console.log("mandando a papi la categoria...");
  emit('cambiarCategorias', category.nombre);
  console.log("categoria enviada",category.nombre);
};
</script>

<style lang="scss" scoped>
@import '../assets/style.scss';
</style>
